 ...
 * Assignment: CS499 Computer Science Capstone, Enhancement 1 software engineering and design by input sanitation of items added to database
  and Enhancement 3 Databases, effectively searching through database to find item by name with software security in mind to prevent SQL injection
 * Student: Amber De La Rosa
 * Recent code update: 4/3/2026
 */
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // create a method that will search for item by name within the database that the recyclerview can access: ENHANCEMENT 3
    public ArrayList<RecyclerView_list> itemSearch(String searchedItem) {
        // open and read from database
        SQLiteDatabase myDb = this.getReadableDatabase();

        // selection constraint for itemName to filter row based on input, this placeholder prevents SQL injection
        String itemQuery = "SELECT * FROM " + InventoryItemTable + " WHERE " + itemName + " LIKE ?";

        // search list for item name based on user input,  use %wild card% to find partial matches
        String[] selectionArgs = {'%' + searchedItem + '%'};


        // use cursor as a pointer for the raw query Selecting item data from the inventory item table
        Cursor cursor = myDb.rawQuery(itemQuery, selectionArgs);


        // create an array list to store items that are found from search
        ArrayList<RecyclerView_list> filteredList = new ArrayList<>();
        ...
        if (cursor.moveToFirst()) {
            ...
            do {
                ...
                //add item to list
                filteredList.add(new RecyclerView_list(id, name, quantity));

                // move cursor to next position while there are more non-empty rows
            } while (cursor.moveToNext());
        }
        // close cursor, close database, and return array list
        cursor.close();
        myDb.close();
        return filteredList;
    }
